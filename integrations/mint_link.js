...
function mintToken(cryptoSecret, callerId, callTime, lang = "en") {
  if (callerId.includes("|") || (lang && lang.includes("|"))) {
    throw new Error("callerId and lang must not contain '|'");
  }
  lang = lang || "en";
  ...
  const plaintext = Buffer.from(`${callerId}|${Math.trunc(callTime)}|${lang}`, "utf8");
...
function mintLink(baseUrl, cryptoSecret, callerId, callTime, lang = "en") {
  const token = mintToken(cryptoSecret, callerId, callTime, lang);
  return `${baseUrl.replace(/\/+$/, "")}/s?t=${token}`;
}

module.exports = { mintToken, mintLink };

// CLI: node mint_link.js --cid +15551234567 [--ts 1717286400] [--lang es] [--base URL]
...
if (require.main === module) {
  ...
  if (!args.cid) { console.error("--cid is required"); process.exit(1); }
  const ts = args.ts ? parseInt(args.ts, 10) : Math.floor(Date.now() / 1000);
  console.log(mintLink(args.base || "https://csat.example.com", secret, args.cid, ts, args.lang || "en"));
}
